    ...
    function jhtmls_build(template) {
        ...
        var lines = template.split(/\n\r?/).map(function (line, index, array) {
            if (/^\s*$/.test(line)) {
                ...
                var expressions_1 = [];
                var offset_1 = 0;
                line.replace(/(!?#)\{((?:"(?:[^\\"]|(?:\\.))*"|'(?:[^\\']|(?:\\.))*'|(?:[^{}]*\{[^}]*\})?|[^}]*)*)\}/g, function (all, flag, value, index) {
                    var text = line.slice(offset_1, index);
                    if (text) {
                        expressions_1.push(JSON.stringify(text));
                    }
                    offset_1 = index + all.length;
                    if (!value) {
                        return '';
                    }
                    // 单纯变量，加一个未定义保护
                    ...
                    switch (flag) {
                        case '#':
                            expressions_1.push('_encode_(' + value + ')');
                            break;
                        case '!#':
                            expressions_1.push('(' + value + ')');
                            break;
                    }
                    return '';
                });
                var text = line.slice(offset_1, line.length);
                if (text) {
                    expressions_1.push(JSON.stringify(text));
                }
                ...
                return '_output_.push(' + expressions_1 + ')';
            }
            else {
                return line;
            }
        ...
        lines.push('}');
        return new Function('_output_', '_encode_', 'helper', lines.join('\n'));
    } /*</function>*/
